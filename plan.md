...
- **Decision (2026-04-23): Option A.** Tokens used both decoratively and functionally must be split at design time (`accent.text` with a text threshold, `accent.decorative` flagged decorative and exempt). Vocabulary-v1 already keeps action and decorative roles separate; teams that reuse a functional token decoratively can declare an `accent.decorative` sibling in their overlay and the audit will flag the decorative uses as candidates for that alias. Option B rejected: runtime discipline smears the contract — a single token reporting one usage can't carry honest receipts for every site that consumes it.
...
- **Decision (2026-04-23): output-format-driven default.** The clipping strategy is derived from the emitted format, not declared as a separate engine knob: sRGB-only outputs (hex, HSL) default to `chroma-reduce` (hue preserved, chroma clipped to sRGB gamut); wide-gamut outputs (P3) default to `chroma-preserve` (keep the original OKLCH chroma, let capable displays show it). Per-token override stays available on `GamutStrategy` for teams that need to force `reject` to fail resolution, but the default no longer requires design-time choice when the target format already implies the right answer.
...
- **Decision (2026-04-23): default to a reference surface, accept the honesty gap.** Alpha tokens default to `bg-main` for light modes and `bg-inverse` for dark modes; declaring a reference surface is optional. Audit will not enforce reference-surface correctness at use sites — the effective contrast of an alpha token on an unexpected surface isn't practically enforceable without tracking every render site, and a strict enforcement regime would block adoption of a scoped post-v1 feature. The receipt records the assumed surface so downstream tooling can surface "this token was audited against bg-main" as metadata rather than a hard constraint.
...
**Current snapshot.** Steps 0-5 shipped. Step 6 phases 1+2+3 shipped (C1), phase 1.5 dual view shipped (C2), phase 4 Tailwind sticker sheet shipped including the CVD axis (C3), phase 5 audit integrator shipped (C4). Audit loop closed end-to-end — `pigmint audit` produces a real `audit-report@0.1` against the sticker-sheet fixture, and the UI consumes it (E1). Ratified ADR wiring landed (G1–G4): ADR-015 gamut default derives from output format, ADR-011 Option A mixed-usage checker ships in the audit, ADR-016 alpha sub-resolver has a pointer-comment in `resolver/driver.ts`, OQ-11 MUI v6 is ratified as the adapter target. All five authoring phases now have a landed surface. Monorepo typecheck + tests green across `core`, `adapter-tailwind` (now 4 tests with the gamut-default assertion), `adapter-mui`, `audit` (12 tests with two new mixed-usage assertions), `authoring-app`, `cli` (sticker-sheet-tailwind carries no tests — visual-only verification; authoring-app at 54 tests, CLI at 14 tests).
...
- **B2. Engine panel parity with `EngineConfig`. [shipped]** The engine panel now exposes the full engine state that makes sense in the authoring app: mode toggles, CVD profile toggles, compliance target, and resolver settings. `intentStore` already carried `engineModes`, `engineCvd`, and `engineResolver`; this pass made them load-bearing in the UI and in `pigmint.yaml` import/export. `pigmintYaml.ts` now round-trips `engine.cvd` and `engine.resolver`, sanitizing unknown CVD entries, canonicalizing mode order, and normalizing resolver config on parse. `ImportPigmintYamlModal.tsx` / `ExportPigmintYamlModal.tsx` now load and save those fields, and CLI `validateProjectConfig` in `packages/cli/src/config.ts` validates `engine.cvd` against the supported profile set so bad configs fail at parse time instead of being silently accepted downstream. Covered by expanded `intentStore.test.ts`, `pigmintYaml.test.ts`, and `packages/cli/tests/config.test.ts`.
- **B4. Per-mode threshold elevation for HC modes. [shipped]** `ModeBinding` now carries an optional `thresholdElevation?: 'hc'` field (`packages/core/src/resolver/driver.ts` + `packages/core/src/resolver/resolve.ts`). When set, `wcagThreshold` bumps the effective floor one WCAG tier: non-text 3:1 → text AA 4.5:1, text AA 4.5:1 → text AAA 7:1, text AAA already at 7 stays put. The CLI's `buildModeBindings` (`packages/cli/src/commands/build.ts`) and the authoring-app's `resolveState.ts` both auto-set `thresholdElevation: 'hc'` when a mode name ends with `-high-contrast`, so builds pick it up without fixture changes. Surface resolution uses role-based indexing, not contrast-picking, so it's unaffected. Verified against the sticker-sheet fixture: `light-high-contrast` now pushes `primary` from `blue.500` (3.17:1) to `blue.600` (5.09:1), borders from `neutral.500` to `neutral.600`, and `muted-foreground` from `neutral.600` (5.01:1 AA-text) to `neutral.700` (8.58:1 AAA-text). Covered by a new `driver.test.ts` assertion that `thresholds.nonText` rises 3 → 4.5 and picked ratios genuinely increase. Approach (1) + (2) from the proposal: a `ThresholdElevation` field on `ModeBinding` for capability, name-based auto-wiring at the edges for ergonomics; the nested-per-mode-intents option (3) is still available if needed but not wired yet.
- **B3. Continuous resolver sampling (opt-in). [shipped]** Resolver mode is now a real engine config surface rather than a hidden implementation detail. `EngineConfig` / `ProjectConfig` carry `engine.resolver: { mode: 'stepped' | 'continuous'; fallbackSteps?: number }`; CLI parsing validates the shape, the authoring app persists it in `intentStore.engineResolver`, and `pigmintYaml.ts` round-trips it. In core, `resolveAll` builds dense working ramps when `mode === 'continuous'` and `scales` are available, then resolves against that denser candidate set while still labeling receipts with the nearest named primitive from the canonical ramp. That means `ResolvedToken.source.position` is no longer constrained to the 11-step primitive grid in continuous mode, while adapter-facing primitives remain stable. The engine panel now exposes a resolver-mode select plus fallback-step input, `ExportModal.tsx`, `SurfacePairViewer.tsx`, and build/export paths honor the configured resolver, and the curve editor's gradient overlay keeps using a continuous working resolver so its marker positions remain load-bearing even when the project is still configured for stepped output. Covered by `packages/core/tests/driver.test.ts`, `packages/authoring-app/tests/intentStore.test.ts`, `packages/authoring-app/tests/pigmintYaml.test.ts`, and CLI config tests.
...
- **C2. Dual view overlays (phase 1.5). [shipped]** Final shape is a two-state `viewMode` toggle on `packages/authoring-app/src/components/curves/CurveOverlayEditor.tsx`: `curves` (default editor — curves, nodes, P3 dashes) and `gradient` (continuous band sampled at 64 points via `buildMonotoneCubicInterpolant` on the smoothed L/C/H values, driven through the same hue-shift + P3 chroma clamp + `oklchToHex` pipeline `generateRamp` uses). Intent markers overlay the gradient view (per user direction: "combine the two"): one pin per resolved vocabulary token whose `source.ramp === scale.name`, positioned by `source.position`, colored by compliance (`AAA*`→green, `AA*`→amber, `exempt`→grey, fail→red). A mode selector (`light` / `dark` / `light-HC` / `dark-HC`, abbreviated from `-high-contrast`) picks which `resolveAll` mode feeds the markers; the selector sits in a dedicated toolbar row above the canvas (not a SVG `foreignObject`) to stop it overlapping step headers. Label collisions are resolved by a greedy row-assignment pass (`rowEndPx` walk), with a thin connector line drawn back to the pin when a label is bumped to a lower row. Earlier `contrast` and `deadweight` modes were dropped — contrast lives in per-step badges elsewhere, and deadweight coverage is a different grain of feedback than a canvas overlay. Shared helper `packages/authoring-app/src/lib/resolveState.ts` still backs both `SurfacePairViewer` and `CurveOverlayEditor` via `runResolve` / `buildTokenRamp` (the latter now delegating to the shared `@pigmint/core` `buildDefaultTokenRamp`, see Vocabulary below).
- **C3. Phase 4 — Sticker sheet (Tailwind). [shipped]** `packages/sticker-sheet-tailwind` renders all four engine modes (`light` / `dark` / `light-high-contrast` / `dark-high-contrast`) side-by-side in a responsive grid, now with a CVD axis layered on top. Adapter fix: `packages/adapter-tailwind/src/emit.ts` `MODE_SELECTORS` for the HC modes moved from `:root[data-contrast="high"]` / `.dark[data-contrast="high"]` (which can only scope to the document root, so HC panels can't coexist with a light panel on the same page) to class-only `.light-high-contrast` / `.dark-high-contrast`; `manifest.ts` `supportedModes` extended to match so `validateAdapterAgainstConfig` stops rejecting the new modes. Fixture `fixtures/pigmint.yaml` now enables all four modes plus `cvd: [deuteranopia, protanopia, tritanopia]`. **CVD axis:** `packages/core/src/emitter/dtcg.ts` now carries `engine.cvd` through into the emitted container (`DtcgContainer.engine.cvd?: CvdProfile[]`), `packages/cli/src/commands/build.ts` passes `config.engine.cvd` through, and `generate:tokens` now writes the list into `tokens.json` under `$extensions.com.pigmint.engine.cvd`. `src/cvd.tsx` is a new module exporting `CvdFilterDefs` (hidden SVG `feColorMatrix` filters using Machado 2009 matrices at severity 1.0 for deuteranopia/protanopia/tritanopia plus a BT.709 luma matrix for achromatopsia), `cvdFilterId(profile)`, and `CVD_PROFILE_LABELS`. `App.tsx` reads the CVD list from `tokens.json`, renders one row per profile (plus a "Normal vision" baseline) with each row applying `filter: url(#pigmint-cvd-<profile>)` to its grid — CVD rows reuse the same four mode panels under the filter so nothing is re-resolved; the filter is purely visual. `src/components/StickerPanel.tsx` covers the full vocabulary surface: Typography (main/muted/subtle foreground), ButtonRow (primary with `--ring` focus outline, secondary on `--border`, ghost on muted, disabled), Card (elevated surface + subtle border + Badge), InputGroup (focus-on-ring), Callout (muted surface), SurfaceInverse (accent + accent-foreground). Visual-only verification; no tests.
- **C4. Phase 5 — Audit integrator. [shipped]** New sixth app mode `'audit'` wired through `App.tsx` + `TopBar.tsx`. `packages/authoring-app/src/store/auditStore.ts` is a Zustand + immer store that holds a loaded `AuditReport` plus a per-suggestion `status: 'pending' | 'accepted' | 'rejected'` map; persisted to `pigmint:audit:v1` with version-check on `artifactVersion === 'audit-report@0.1'`. Actions: `loadReport`, `loadReportFromText` (JSON parse + shape validation, returns a discriminated result), `clearReport`, `setStatus`, `resetStatuses`. Exports `groupByToken(report)` that merges violations + top-level suggestions + violation-nested suggestions into `GroupedEntry[]` sorted alphabetically by token path. `packages/authoring-app/src/components/audit/AuditIntegrator.tsx` hosts the UI: hidden `<input type="file">` opened via an "Open report…" button, `ReportHeader` with run ID / timestamp / profile / stat pills, per-token `TokenCard`s that list violations (severity pill + type + mode + actual ratio) and `SuggestionRow`s (channel + confidence + rationale + change label + Accept/Reject/Reset). `applySuggestion()` maps `change.field` (`defaultIntent.preference`, `.consistency`, `.surfaceContext`) to `useIntentStore` setters via type guards (`isPreference`, `isConsistency`, `isSurfaceContext`); ramp-suggestion and spec-gap channels surface read-only per the three-channel design — Accept on those records a rejected status with an explanatory error banner. `@pigmint/audit` is imported as types only (`import type { … }`) so the authoring app doesn't pull `node:crypto` into the browser bundle. Covered by `packages/authoring-app/tests/auditStore.test.ts` (10 tests: load/parse/persist/status round-trips, groupByToken merging and sorting) and browser-verified end-to-end by loading a sample `audit-report@0.1` JSON, accepting an `intent-refinement` suggestion, and confirming it lands in `useIntentStore.overrides`.

### E. Audit loop closure

- **E1. Tie the audit CLI + integrator together end-to-end. [shipped 2026-04-24]** The `audit` subcommand in `packages/cli/src/commands/audit.ts` was already wired through `packages/cli/src/bin.ts` (loads config, reads emitted DTCG, calls `auditContainer`, writes `audit-report@0.1` JSON, exits non-zero on `error`-severity violations) but wasn't exposed as a workspace export or demonstrated against a real fixture. Closed by (1) adding `export { audit, type AuditOptions, type AuditRunResult }` to `packages/cli/src/index.ts` so programmatic consumers can invoke it without going through `bin.js`; (2) adding an `audit:` block to `packages/sticker-sheet-tailwind/fixtures/pigmint.yaml` (`report: ../src/generated/audit.json`, `profile: wcag-srgb`) so the generated report lands next to `tokens.json`; (3) adding an `audit` script to `packages/sticker-sheet-tailwind/package.json` (`cd fixtures && node ../../cli/dist/bin.js audit`, invoked via `pnpm --filter @pigmint/sticker-sheet-tailwind run audit` because bare `pnpm audit` collides with pnpm's builtin). Running the CLI against the fixture produces a real `audit-report@0.1` with 4 `intent-refinement` warnings on `color.foreground.subtle` (light, dark, light-high-contrast, dark-high-contrast — the ramp's subtle step passes AA-nonText but target asks for AA-text), each carrying a `defaultIntent.preference → highest-contrast` suggestion that the UI's `applySuggestion` handles directly. The full loop is now: `pnpm --filter @pigmint/sticker-sheet-tailwind run audit` → `src/generated/audit.json` → open in authoring-app's Audit panel → Accept writes to `useIntentStore.overrides` → export `pigmint.yaml` → next build picks up the override. `packages/cli/tests/end-to-end.test.ts` already covers the CLI half (audit run writes a schema-valid report; subsequent build exposes `priorAudit.suggestions`); the UI half was browser-verified in C4.

### G. Ratified ADR wiring

- **G1. ADR-015 format-driven gamut default. [shipped]** `buildResolvedValue` in `packages/core/src/resolver/resolve.ts` now emits `value.oklch` directly from `step.oklch` (the P3-clamped full-chroma value from `generateRamp`) instead of round-tripping through `step.hex` — previously both `format: oklch` and `format: hex` output emerged from the sRGB-clamped hex, collapsing the gamut distinction. `hex` stays sRGB-reduced (culori's `clampChroma(..., 'oklch')`); `oklch` preserves chroma up to P3. `emitDtcg` records the derived gamut strategy per mode on `ModeEntry.gamut.strategy` — `chroma-preserve` when the step sits in sRGB or when the token intent pins it, `chroma-reduce` otherwise. `packages/adapter-tailwind/src/emit.ts` derives the per-output default from the format array (`hex` → reduce, `oklch` → preserve) and stamps it into the CSS header comment alongside format + preset + default mode. Per-token `IntentConstraints.gamutStrategy` still acts as an override. Covered by a new `records format-driven gamut default in header` test in `packages/adapter-tailwind/tests/emit.test.ts`; regenerated `packages/sticker-sheet-tailwind/src/generated/tokens.css` confirms the output now reads `/* ... gamut: chroma-preserve; ... */` with oklch chromas sourced from the curve values rather than a hex roundtrip (e.g. `--primary` shifted from `0.1642281918046476` to `0.1644744176198368`).
- **G2. ADR-011 Option A mixed-usage checker. [shipped]** `packages/audit/src/checkers.ts` now carries a `PATH_RULES` table and `pathContextFor(tokenPath)` that infers the expected usage (`text` / `nonText` / `decorative`) from the token path's category or leaf segment (`decorative.*` → decorative; `foreground.*` / `*.text` / `*.label` → text; `surface.*`, `border.*`, `focus.*`, `*.background` → nonText). When the declared vocabulary usage disagrees with the path-implied usage, it emits a `mixed-usage` warning violation. `packages/audit/src/suggestions.ts` now recognises the new type and emits a `spec-gap`-channel suggestion recommending the token be split at design time per ADR-011 Option A — a single token can't serve both text-contrast duty and decorative visual duty without compromise. Covered by two new tests in `packages/audit/tests/checkers.test.ts`: one asserts `color.decorative.backdrop` declared as `text` is flagged with the expected/actual swapped, and one asserts a path-consistent `color.foreground.main: text` produces no mixed-usage violation. All 12 audit tests pass.
- **G3. ADR-016 alpha sub-resolver pointer. [shipped]** Added a doc-block at the top of `packages/core/src/resolver/driver.ts` that pins the default reference-surface policy ratified in ADR-016: alpha-composited tokens default to `color.surface.main` (light schemes) / `color.surface.inverse` (dark schemes), adapter config's `alpha.referenceSurface` overrides when present, and audit records the assumed reference as metadata rather than enforcing compliance at use sites. No sub-resolver is shipping in v1 — the pointer-comment is there so the next engineer (or future-me) finds the decision and the entry point together rather than re-deriving it from the plan.
- **G4. OQ-11 MUI v6 target ratified. [shipped]** Updated `plan.md` OQ-11 to a **Resolved: v6** section — the adapter assumes stable `CssVarsProvider`, `extendTheme`, and `theme.applyStyles()`, and emits a single theme object with `colorSchemes: { light, dark, ... }` rather than parallel `ThemeProvider` objects. No v5 fallback is maintained. `packages/adapter-mui/src/manifest.ts` already pinned `presets: ['mui-v6']`; added a comment block explaining the ratification alongside the manifest so the decision sits next to the code. v7 upgrade path stays cheap (same CssVarsProvider contract), so locking v6 as the minimum doesn't foreclose moving forward later.

### F. Open design questions

- **F1. Materialize fractional continuous-mode picks as primitives.** Today in continuous resolver mode (`engine.resolver.mode: 'continuous'`, B3), `source.position` is a fractional number (e.g. `0.443`) but the emitted `$value` alias round-trips through `source.nearestPrimitive` to the nearest stepped primitive name (e.g. `{color.primitive.neutral.600}`). The receipt carries the exact position but the adapter-facing alias is a small approximation — the actual oklch differs from the named step's oklch. The `CurveOverlayEditor` marker labels expose this (shows `Indigo 443` interpolated between step names, while tokens.json binds to `indigo.500`). Proposal: when continuous mode is active, collect the unique `(ramp, position)` tuples across all resolved semantic tokens, synthesize a primitive per tuple with a deterministic interpolated name (`${ramp}.${round(loNum + (hiNum - loNum) × frac)}`), emit those primitives into the primitives tree alongside the stepped set, and rewrite `nearestPrimitive` to point at the materialized name. Tradeoffs: (1) **dedup** — need a rounding strategy so near-identical positions share a primitive rather than each spawning their own (round to nearest 10? 25?); (2) **non-numeric step names** — presets like shadcn use `background` / `border` rather than `100..900`, where interpolated naming breaks; need a fallback or a preset gate; (3) **adapter impact** — `packages/adapter-tailwind/src/emit.ts` iterates the stepped primitives to emit CSS vars; non-standard numeric primitives propagate through as new CSS var names and need explicit handling; (4) **audit and sticker-sheet** consume primitive keys too. Not blocking; stepped mode (the default) is unaffected. Builds on B3 and keeps the receipt-as-truth principle (ADR-009) honest by removing the last approximation in the resolved → emitted path.

### D. Hygiene

- **D1. Replace palette-pal's seed palette. [shipped]** `packages/authoring-app/src/color-tokens.json` already held an empty `scales: []` seed; confirmed the app opens straight to `BulkCreatePanel` on a fresh profile. No code change needed — the concern was stale.
- **D2. Drop any remaining palette-pal references. [shipped]** Grep sweep across `packages/authoring-app/src/` and `tests/` — the only surviving `palette-pal` references are the load-bearing legacy-storage migration key (`palette-pal:color-tokens` in `store/paletteStore.ts` + its `migrateLegacyStorage.test.ts`). Those stay: removing the string would break A4's migration for users mid-fork.
- **D3. Engine `modes` defaulting. [shipped]** Defaults bumped from `['light']` to `['light', 'dark']` in both `packages/authoring-app/src/store/intentStore.ts` (`DEFAULT_STATE.engineModes`, `sanitizeModes` fallback) and `packages/authoring-app/src/lib/pigmintYaml.ts` (`DEFAULT_ENGINE.modes`). `toggleEngineMode` now explicitly short-circuits when removing would leave zero modes rather than relying on `sanitizeModes([])`'s fallback (which used to re-seed `['light']` and silently disagreed with the new default). Tests in `intentStore.test.ts` + `pigmintYaml.test.ts` updated to the new default.
- **D4. Vocabulary + default token-ramp mapping. [shipped]** `VOCABULARY_V1_SLICE` in `packages/core/src/defaults/vocabulary-v1.ts` expanded from 4 to 13 tokens covering surface (`main` / `elevated` / `subtle` / `inverse`), foreground (`main` / `muted` / `subtle` / `inverse`), action (`primary.background` / `primary.text`), border (`main` / `subtle`), and focus (`ring`). Every entry is constrained to resolver-supported shapes (`consistency: 'independent'`, base state only, preferences in `{lowest-passing, highest-contrast}`) — spec/09's `matched-to-set` / `matched-across-ramps` entries deliberately deferred until the resolver grows them. The ramp-classification heuristic used by the CLI, `SurfacePairViewer`, and `CurveOverlayEditor` previews was extracted into a shared `buildDefaultTokenRamp(vocab, rampNames)` helper exported from `@pigmint/core` (`packages/core/src/defaults/token-ramp.ts`); callers in `packages/cli/src/commands/build.ts` and `packages/authoring-app/src/lib/resolveState.ts` now delegate to it, and driver/adapter tests were updated to seed via the same helper (`byKey.size` assertion now tracks `VOCABULARY_V1_SLICE.length * 2` rather than a hard-coded 8).
...
**Embedded decisions inside accepted ADRs — resolved 2026-04-23:**

- **ADR-011 (spec composition): Option A.** Split mixed-usage tokens at design time. Functional and decorative roles never share a token; teams that currently double-dip on an accent must declare an `accent.decorative` sibling and let the audit flag decorative uses against it.
- **ADR-015 (output format plurality): format-driven default.** sRGB-only outputs default to `chroma-reduce`, P3 outputs default to `chroma-preserve`. Per-token override stays available; no separate engine-level default knob is needed.
- **ADR-016 (alpha tokens): default reference surface, no enforcement.** Alpha tokens default to `bg-main` (light modes) / `bg-inverse` (dark modes); declaration is optional. Audit records the assumed reference surface as metadata but does not enforce correctness at use sites — reference-surface compliance at render time is not practically enforceable without site-level tracking.

- **OQ-11 (MUI version target): v6 ratified.** Adapter assumes stable CssVarsProvider and `colorSchemes`; no v5 fallback. See OQ-11 below for full rationale.

Remaining open questions: OQ-1 through OQ-10 below (OQ-11 resolved; OQ-12 resolved).
...
**Resolved: v6 is the adapter target.** The adapter assumes stable `CssVarsProvider`, `extendTheme`, and `theme.applyStyles()`. Output shape is a single theme object with `colorSchemes: { light, dark, ... }` and matching per-mode receipts — not two parallel `ThemeProvider` objects. `@pigmint/adapter-mui` manifest already pins `presets: ['mui-v6']`; no v5 fallback is maintained. v7 upgrade path stays cheap (same CssVarsProvider contract), so locking v6 as the minimum doesn't foreclose moving forward later. Re-examine only if the author's work becomes permanently stuck on v5.
