...
- **Phase 1 (ramp authoring with dual view). Shipped (basic fork parity).** Palette-pal forked into `packages/authoring-app`, rewired to `@pigmint/core` for all math. Dual view overlays (contrast bands, deadweight indicator, intent markers) remain deferred — tracked as action item C2.
...
**Current snapshot.** Steps 0-5 shipped. Step 6 phases 1+2 shipped. Monorepo typecheck + tests green across `core`, `adapter-tailwind`, `adapter-mui`, `audit`, `authoring-app`, `cli` (55 tests total).
...
- **C2. Dual view overlays (phase 1.5).** Deferred from phase 1. Adds to the existing CurveOverlayEditor: (i) contrast band overlay on the continuous gradient (“this region passes AA-text against white”), (ii) deadweight indicator for curve regions with no intent/primitive coverage, (iii) intent-resolved markers showing where each vocabulary entry landed post-resolve. Requires the A-group work first, since intent markers only make sense once the resolver reads intents.
